...
struct local
{
	...
	CloudConstPtr    getLatestCloud ()
	{
		...
		CloudPtr cloud_floor (new Cloud);
		CloudPtr cloud_wall (new Cloud);
		std::vector<pcl::PointIndices> cluster_indices;
		bool planeWall(false);
		...
		extract_.setNegative (planeWall);
		...
		if(maxPlane.y > 0.15) 
		{
			planeWall = true; //Plane is wall
			*cloud_wall = *cloud_plane;
		}
		else
		{
			*cloud_floor = *cloud_plane; //Plane is floor
		}

		if(planeWall)
		{
			extract_.setNegative (planeWall);
			extract_.filter (*temp_cloud3);

			seg_.setInputCloud (temp_cloud3);
			seg_.segment (*inliers, *coefficients);
			

			////
			extract_.setInputCloud (temp_cloud3);
			extract_.setIndices (inliers);
			extract_.setNegative (false);
			extract_.filter (*cloud_floor);
	...
	//	delete[] &matObsInfo[1];
		...
		viewer->removePointCloud("cloud 4");
	...
	{	
		...
		for(int i = 0; i < numObs; ++i)
		{
			...
			{
				if(matObsInfo[0][i].z < 1.2) checkPath++;
				if(matObsInfo[0][i].z < 0.9) checkObstacle++;
		...
		viewer->addPointCloud<CloudType>  (cloud_floor, single_color3, "cloud 3" );	

		if(planeWall)
		{
			pcl::visualization::PointCloudColorHandlerCustom<CloudType> single_color4(cloud_plane, 255, 255, 0);
			viewer->addPointCloud<CloudType>  (cloud_wall, single_color4, "cloud 4" );	
		}
		...
		if(planeWall) viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "cloud 4");
			 ...
			 else 
			{
				...
				else
				{
					...
					else 
					{
						...
						CString Dir(_T(""));
						double temp1 = rotateBack(angleTemp, m_dbAngleCurrent, Dir);
						sendCommand(Dir, temp1);
						waitRobot();
...
void CRobotVisionDlg::OnBnClickedButtonRun()
{
	...
	seg_.setDistanceThreshold (0.035); //2cm
	//	extract_.setNegative (true);
...
//----------------------------------------------------------------------------------------
//rotate robot back to previous angle
double CRobotVisionDlg::rotateBack(const double angleTurn, const double angleCur, CString &Dir)
{
	if(angleCur >= angleTurn && angleCur < (angleTurn + 180))
	{
		Dir = "R";
		return (angleCur - angleTurn);
	}
	else if(angleCur >= (angleTurn + 180) && angleCur <= 360)
	{
		Dir = "L";
		return (360 - angleCur + angleTurn);
	}
	else if(angleCur >= 0 && angleCur <= angleTurn)
	{
		Dir = "L";
		return (angleTurn - angleCur);
	}
}
